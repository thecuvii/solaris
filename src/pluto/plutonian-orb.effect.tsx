...
export function PlutonianOrbEffect({
  ...
  useEffect(() => {
    ...
    function render(timestamp: number): void {
      ...
      const elevation = (clamp(current.sunElevation, -30, 90) * Math.PI) / 180
      ...
      const rotation =
        (wrapDegrees(current.surfaceRotation) * Math.PI) / 180 +
        elapsed * clamp(current.rotationSpeed, 0, 0.05)
      ...
      gl.uniform1f(
        ...
        clamp(current.iceResponse, 0, 2),
      )
      gl.uniform1f(
        gl.getUniformLocation(resources.program, 'uPhaseFill'),
        clamp(current.phaseFill, 0, 0.25),
      )
      gl.uniform2f(
      ...
      gl.uniform1f(
        ...
        clamp(current.reliefStrength, 0, 2),
      )
      gl.uniform2f(
      ...
      gl.uniform1f(
        ...
        clamp(current.roughness, 0.35, 1),
      )
      ...
      gl.uniform1f(
        ...
        clamp(current.tholinStrength, 0, 1.5),
      )
      gl.uniform1f(
        gl.getUniformLocation(resources.program, 'uViewTilt'),
        (clamp(current.viewTilt, -30, 30) * Math.PI) / 180,
      )
