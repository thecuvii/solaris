...
- Relative airmass uses the Kasten & Young (1989) formula; per-channel Rayleigh, aerosol, and Chappuis-band ozone zenith optical depths are order-of-magnitude sea-level values evaluated per disc row so the low disc grades from yellow to red. The in-disc gradient is editorially stretched (`DISC_GRADIENT_STRETCH`). `duskFlush` is an editorial magenta grade on the lower disc and its glow; it is not a measured Chappuis spectrum. `field` at 0 replaces the scattering sky with a dark indigo plate.
