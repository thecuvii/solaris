...
export const parameterDefinitions: Record<PlanetId, readonly ParameterDefinition[]> = {
  ...
  'observed-sun': [
    unit('cloudStreaks', 0.3, { group: 'atmosphere', label: 'Cloud streaks' }),
    unit('duskFlush', 0, { group: 'atmosphere', label: 'Dusk flush' }),
    number('exposure', 1, 0.2, 8, 0.05, { group: 'lighting', label: 'Exposure' }),
    unit('field', 1, { group: 'atmosphere', label: 'Field' }),
    amount('glare', 0.4, { group: 'lighting', label: 'Glare' }),
    unit('haze', 0.5, { group: 'atmosphere', label: 'Haze' }),
    amount('ozone', 1, { group: 'atmosphere', label: 'Ozone' }),
    number('saturation', 1, 0, 2, 0.01, { group: 'lighting', label: 'Saturation' }),
    number('refraction', 1, 0, 1.5, 0.01, { group: 'pose', label: 'Refraction' }),
    unit('seeingAmount', 0.35, { group: 'motion', label: 'Seeing' }),
    ...
    sunElevation(2, 'sunElevation', { max: 70, min: -1 }),
    number('sunScale', 0.36, 0.1, 0.6, 0.01, { group: 'pose', label: 'Disc size' }),
  ],
  pluto: [
    exposure(1),
    lean,
