...
const frames = `${output}/frames-v29-4k60`
...
try {
  ...
  for (
    ...
    if (frame >= Math.round(2.2 * fps) && frame <= 4 * fps) {
  ...
  await copyFile(`${frames}/00030.png`, `${output}/solaris-launch-poster-v29-4k60.png`)
  await writeFile(
    `${output}/render-report-v29-4k60.json`,
    JSON.stringify(
      {
        width: 3840,
        height: 2160,
...
const encoder = spawn(
  ...
  [
    ...
    '-movflags',
    '+faststart',
    `${output}/solaris-launch-opening-v29-4k60.mp4`,
  ],
  { stdio: 'inherit' },
)
const [code] = await once(encoder, 'exit')
if (code !== 0) throw new Error(`ffmpeg exited ${code}`)
console.log(`${output}/solaris-launch-opening-v29-4k60.mp4`)
